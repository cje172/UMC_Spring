...
public class UserDao {
    ...
    public int createUser(PostUserReq postUserReq) {
        String createUserQuery = "insert into User (email, password, username, profileIntro, profileImgUrl) VALUES (?,?,?,?,?)"; // 실행될 동적 쿼리문
        Object[] createUserParams = new Object[]{postUserReq.getEmail(), postUserReq.getPassword(), postUserReq.getUsername(), postUserReq.getProfileIntro(), postUserReq.getProfileImgUrl()}; // 동적 쿼리의 ?부분에 주입될 값
...
//    }
    ...
    // 프로필 편집
    public int modifyProfile(PatchUserReq patchUserReq) {
        String modifyUserNameQuery = "update User set username = ?, profileIntro = ?, profileImgUrl = ? where userIdx = ? "; // 해당 userIdx를 만족하는 User를 해당 nickname으로 변경한다.
        Object[] modifyUserNameParams = new Object[]{patchUserReq.getUsername(), patchUserReq.getProfileIntro(), patchUserReq.getProfileImgUrl(), patchUserReq.getUserIdx()}; // 주입될 값들(nickname, userIdx) 순
    ...
    public User getPwd(PostLoginReq postLoginReq) {
        String getPwdQuery = "select userIdx, email, password, username, profileIntro, profileImgUrl from User where email = ?"; // 해당 email을 만족하는 User의 정보들을 조회한다.
